...
class TbButtonColumn extends CButtonColumn {
    ...
    protected function renderButton($id, $button, $row, $data) {
        ...
        if (isset($button['iconExp'])) {
            $button['icon'] = $this->evaluateExpression($button['iconExp'], array('data' => $data, 'row' => $row));
        }
        ...
        if (isset($button['icon'])) {
            ...
            $image = '<i class="' . $button['icon'] . '"></i>';

        } else if (isset($button['imageUrl']) && is_string($button['imageUrl'])) {
            $image = CHtml::image($button['imageUrl'], $label);
        } else {
            $image = null;
        }

        if(isset($button['iconWithText'])) {
            $linkText = $image . '&nbsp;' . $label;
        }
        else {
            $linkText = $image ? $image : $label;
        }

        echo CHtml::link($linkText, $url, $options);
    }

    protected function renderFilterCellContent()
    {
        $onclick = "$('#".$this->grid->getId()." input').change();";
        echo '<button type="button" onclick="'.$onclick.'" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-search"></i></button>';
    }
