(function()
{
            ...
            $.post($frm.attr('action'), $frm.serialize(), function(response) {
                ...
                if(1 === response.error) {
                    ...
                    if(response.invalidated) {
                        self.showValidationErrors($frm, response.invalidated);
                    }
    ...
    Form.prototype = 
    {
        showValidationErrors: function($frm, errors) {
            var $error, fieldErrors;
                        
            for(var field in errors) {
                fieldErrors  = errors[field];

                var $input = $('*[name=' + this.options.modelToFormFieldName(field) + ']', $frm),
                    $error = [];

                for(var rule in fieldErrors) {
                    $error.push(fieldErrors[rule]);
                }

                $input.closest('.form-group').addClass('has-error');

                if(this.options.showErrorDescription) {

                    // mode: toolbox
                    if(this.options.errorDescription === 'toolbox') {
                        $input.tooltip({
                                title: $error.join(', '),
                                placement: 'top',
                                trigger: 'manual'
